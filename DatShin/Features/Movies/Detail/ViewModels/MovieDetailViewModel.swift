...
class MovieDetailViewModel {
    ...
    @Published private(set) var similarMovies: [MovieListItem] = []
    ...
    init(id: Movie.ID, fetcherService: MovieServiceProtocol) {
        self.id = id
    ...
    struct MovieDetail {
        ...
        let similarMovies: MoviePageableList
    ...
    func fetchData() {
        Task {
            do {
                self.isLoading = true
                async let movieData = fetcherService.details(forMovie: id, language: nil)
                async let showCreditsData = fetcherService.credits(forMovie: id, language: nil)
                async let similarMoviesData = fetcherService.similar(toMovie: id, page: nil, language: nil)
                async let watchProvidersData = fetcherService.watchProviders(forMovie: id, country: "us")
                ...
                self.similarMovies = movieDetail.similarMovies.results
